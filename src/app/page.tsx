...
import { TrendingUp, TrendingDown, Wallet, Target, ArrowUpRight, ArrowDownRight, Clock, AlertCircle, Plus, Trash2 } from 'lucide-react';
import { getIncomes, getExpenses, getBudgetItems, getProspectiveExpenses, saveProspectiveExpense, deleteProspectiveExpense } from '@/lib/db';
...
export default function Dashboard() {
  ...
  const refresh = async () => {
    const [inc, exp, bud, pro] = await Promise.all([
      getIncomes(month), getExpenses(month), getBudgetItems(month), getProspectiveExpenses(month)
    ]);
    setIncomes(inc); setExpenses(exp); setBudget(bud); setProspective(pro);
  };

  useEffect(() => { refresh(); }, [month]);

  const handleAddProspective = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!proDesc.trim() || !proAmountUSD) return;
    await saveProspectiveExpense({ description: proDesc.trim(), amount: proAmountUSD, month });
    setProDesc(''); setProAmountUSD(0);
    refresh(); playSuccess();
  };

  const handleDeleteProspective = async (id: string) => {
    await deleteProspectiveExpense(id); refresh(); playDelete();
  };
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {stats.map((stat, i) => {
          ...
          return (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
              className="glass-card card-hover p-5" style={{ border: `1px solid ${stat.border}` }}>
              <div className="flex items-start justify-between mb-4">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: stat.bg }}>
                  <Icon className="w-5 h-5" style={{ color: stat.color }} />
                </div>
                <span className="text-xs px-2 py-1 rounded-full" style={{ background: stat.bg, color: stat.color }}>{stat.change}</span>
              </div>
              ...
              {rate && <p className="text-xs mt-1" style={{ color: 'rgba(246,211,101,0.55)' }}>{fmtBs(usdToBs(stat.value))}</p>}
            </motion.div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }} className="glass-card p-6">
          ...
          <div className="space-y-5">
            {[
              { label: 'Ingresos', val: totalIncome, max: totalIncome, c1: '#43e97b', c2: '#38f9d7', tc: 'text-emerald-400', delay: 0.5 },
              { label: 'Gastos', val: totalExpenses, max: totalIncome, c1: '#fa709a', c2: '#fee140', tc: 'text-pink-400', delay: 0.6 },
              { label: 'Presupuesto Ejecutado', val: budgetSpent, max: totalBudget, c1: '#f6d365', c2: '#fda085', tc: 'text-yellow-400', delay: 0.7 },
            ].map((bar) => (
              <div key={bar.label}>
                <div className="flex justify-between mb-2">
                  <span className="text-sm" style={{ color: 'rgba(232,234,246,0.55)' }}>{bar.label}</span>
                  <span className={`text-sm font-semibold ${bar.tc}`}>{formatCurrency(bar.val)}</span>
                </div>
                <div className="w-full h-2.5 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
                  <motion.div initial={{ width: 0 }}
                    ...
                    className="h-full rounded-full" style={{ background: `linear-gradient(90deg, ${bar.c1}, ${bar.c2})` }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.38 }}
        className="glass-card p-6" style={{ border: '1px solid rgba(253,160,133,0.18)' }}>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          <div>
            <form onSubmit={handleAddProspective} className="space-y-3">
              <input type="text" placeholder="Ej: Renta, Seguro, Viaje..." value={proDesc}
                onChange={(e) => setProDesc(e.target.value)} className="w-full" required />
              <CurrencyInput valueUSD={proAmountUSD} onChange={setProAmountUSD} placeholder="Monto estimado" required />
              <button type="submit" className="btn-glow w-full flex items-center justify-center gap-2 py-2.5 rounded-xl font-semibold text-sm text-white"
                style={{ background: 'linear-gradient(135deg, #fda085 0%, #f6d365 100%)' }}>
                <Plus className="w-4 h-4" /> Agregar gasto probable
              </button>
            </form>
            <div className="mt-4 space-y-2">
              <AnimatePresence>
                {prospective.length === 0 ? (
                  <p className="text-sm text-center py-4" style={{ color: 'rgba(232,234,246,0.25)' }}>Aún no hay gastos probables</p>
                ) : prospective.map((p) => (
                  <motion.div key={p.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }}
                    ...
                    <button onClick={() => handleDeleteProspective(p.id)} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-white/10">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="p-4 rounded-xl" style={{
              ...
              {balanceAfterProspective < 0 && <p className="text-xs mt-2 text-pink-400">Cuidado: tus gastos probables superan tu saldo</p>}
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.42 }} className="glass-card p-6">
        ...
        {recentTransactions.length === 0 ? (
          <p className="text-center py-8" style={{ color: 'rgba(232,234,246,0.25)' }}>Sin movimientos este mes. ¡Comienza registrando un ingreso!</p>
        ) : (
              ...
              <motion.div key={tx.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center"
                    ...
                    {tx.type === 'income' ? <ArrowUpRight className="w-4 h-4 text-emerald-400" /> : <ArrowDownRight className="w-4 h-4 text-pink-400" />}
                  </div>
                  <div>
                    <p className="text-sm font-medium text-white">{tx.description}</p>
                    <p className="text-xs" style={{ color: 'rgba(232,234,246,0.38)' }}>{tx.category} · {new Date(tx.date).toLocaleDateString('es-VE')}</p>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  {rate && <p className="text-xs" style={{ color: 'rgba(246,211,101,0.5)' }}>{fmtBs(usdToBs(tx.amount))}</p>}
                </div>
              </motion.div>
            ))}
          </div>
