...
import { getIncomes, saveIncome, deleteIncome, getCurrentMonth, formatCurrency, formatMonth } from '@/lib/store';
...
export default function IngresosPage() {
  ...
  const load = () => setIncomes(getIncomes().filter((i) => i.month === selectedMonth));
  useEffect(() => { load(); }, [selectedMonth]);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.description || !amountUSD) return;
    saveIncome({ id: uuidv4(), ...form, amount: amountUSD, month: selectedMonth });
  ...
  const handleDelete = (id: string) => { deleteIncome(id); load(); playDelete(); };
      ...
      <AnimatePresence>
        {showForm && (
          <motion.div
            ...
            style={{ border: '1px solid rgba(67,233,123,0.25)' }}
          >
